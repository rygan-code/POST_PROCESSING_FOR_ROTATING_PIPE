program post
  ...
  integer:: num,i,j,k,m,n,id,id_y,id_x,id_z,ierr
  
  call mpi_init(ierr)                       ! initial of MPI
  call mpi_comm_rank(MPI_COMM_WORLD,id,ierr)   ! get my_id
...
subroutine wavelet_transform(ny,x,y,z,fin,fout)
  ...
  do i = 1 , ny-1   ! scale
    ...
    do j = 1, ny/2    ! position
      position = r(1,j*2,1)
      psi(:,i,j) = mexican_hat_wavelet((r(1,:,1)-position)/scale)
      fout(i,j) = sum(scale**(-0.5)*psi(:,i,j)*fin(:))
    end do
  end do
end subroutine
