program post
  ...
  real*8,allocatable,dimension(:,:)::d_av_x, u_av_x, v_av_x, w_av_x, T_av_x,energy,helicity,energy_sum,helicity_sum
  ...
  integer:: num,i,j,k,m,n,id_y,id_x,id_z,ierr
  
  call mpi_init(ierr)                       ! initial of MPI
  call mpi_comm_rank(MPI_COMM_WORLD,my_id,ierr)   ! get my_id
  if(my_id.eq.0)print*, "Post-processing code for OpenCFD2,  output tecplot-type data"
  ! read(*,*) nx,ny,nz
  ...
  if(my_id.eq.0)print*, " nx= ",nx," ny= ",ny," nz= ",nz
  LAP = 0
  ...
  call read_mesh(nx,ny,nz,Axx(1:nx,1:ny,1:nz),Ayy(1:nx,1:ny,1:nz),Azz(1:nx,1:ny,1:nz),my_id)
  call comput_Jacobian3d

  open(77,file="opencfd.dat",form="unformatted")
  read(77) Istep
  read(77) tt
  if(my_id.eq.0) print*, "Istep, tt=", Istep, tt

  call read3d(77,nx,ny,nz,d,my_id)
  call read3d(77,nx,ny,nz,u,my_id)
  call read3d(77,nx,ny,nz,v,my_id)
  call read3d(77,nx,ny,nz,w,my_id)
  call read3d(77,nx,ny,nz,T,my_id)
  close(77)

  call test_uiuj
  ! if(my_id.eq.0)print*,-sum((u*u+v*v+w*w)/Ajac)
  ...
  ! wyy = 0
  ! wxx(256,128,1) = 1
  ! call poisson_solver_gaussian_seidel(wxx,ux_induced,wyy)
  ! call poisson_solver_gaussian_seidel(wxz-wzx,uy_induced,v)
  ! call poisson_solver_gaussian_seidel(wyx-wxy,uz_induced,w)
  ! call write_3D(nx,ny,nz,Axx(1:nx,1:ny,1:nz),Ayy(1:nx,1:ny,1:nz),Azz(1:nx,1:ny,1:nz),d,ux_induced,uy_induced,uz_induced,T,P00,Ma,Q_criterion)
...
!-------------------------- Fourier analysis----------------
  ...
  ! allocate(u_wavelet(ny-1,ny/2))
  ! allocate(v_wavelet(ny-1,ny/2))
  ! allocate(w_wavelet(ny-1,ny/2))
  ! allocate(vor_x_wavelet(ny-1,ny/2))
  ! allocate(vor_y_wavelet(ny-1,ny/2))
  ! allocate(vor_z_wavelet(ny-1,ny/2))
  ! allocate(energy(ny-1,ny/2))
  ! allocate(helicity(ny-1,ny/2))
  ! allocate(energy_sum(ny-1,ny/2))
  ! allocate(helicity_sum(ny-1,ny/2))
  ! ! call fourier_transform(nx,ny,nz,Axx(1:nx,1:ny,1:nz),Ayy(1:nx,1:ny,1:nz),Azz(1:nx,1:ny,1:nz),Q_criterion,Q_criterion_f)
  
  ! id_x = my_id/8
  ! id_z = mod(my_id,8)
  ! do m=1, nx!/8
  !   do n=1, nz!/8
  !     call wavelet_transform(ny,Axx(m+8*id_x,:,n+8*id_z),Ayy(m+8*id_x,:,n+8*id_z),&
  !                           Azz(m+8*id_x,:,n+8*id_z),u(m+8*id_x,:,n+8*id_z),u_wavelet)
  !     ! if(my_id.eq.0)print*,u_wavelet
  !     call wavelet_transform(ny,Axx(m+8*id_x,:,n+8*id_z),Ayy(m+8*id_x,:,n+8*id_z),&
  !                           Azz(m+8*id_x,:,n+8*id_z),v(m+8*id_x,:,n+8*id_z),v_wavelet)
  !     call wavelet_transform(ny,Axx(m+8*id_x,:,n+8*id_z),Ayy(m+8*id_x,:,n+8*id_z),&
  !                           Azz(m+8*id_x,:,n+8*id_z),w(m+8*id_x,:,n+8*id_z),w_wavelet)
  !     call wavelet_transform(ny,Axx(m+8*id_x,:,n+8*id_z),Ayy(m+8*id_x,:,n+8*id_z),&
  !                           Azz(m+8*id_x,:,n+8*id_z),vor_x(m+8*id_x,:,n+8*id_z),vor_x_wavelet)
  !     call wavelet_transform(ny,Axx(m+8*id_x,:,n+8*id_z),Ayy(m+8*id_x,:,n+8*id_z),&
  !                           Azz(m+8*id_x,:,n+8*id_z),vor_y(m+8*id_x,:,n+8*id_z),vor_y_wavelet)
  !     call wavelet_transform(ny,Axx(m+8*id_x,:,n+8*id_z),Ayy(m+8*id_x,:,n+8*id_z),&
  !                           Azz(m+8*id_x,:,n+8*id_z),vor_z(m+8*id_x,:,n+8*id_z),vor_z_wavelet)
      
  !     ! print*, m+8*id_x,n+8*id_z
  !     do i = 1 , nx/8
  !       do j = 1, nz/8
  !         energy = energy - (u_wavelet*u_wavelet+v_wavelet*v_wavelet+&
  !                           w_wavelet*w_wavelet)*sum(1/Ajac(m+8*id_x,:,n+8*id_z))
  !         helicity = helicity - (u_wavelet*vor_x_wavelet+v_wavelet*vor_y_wavelet+&
  !                               w_wavelet*vor_z_wavelet)*sum(1/Ajac(m+8*id_x,:,n+8*id_z))
  !       end do
  !     end do
  !   end do
  ! end do
  ! call mpi_reduce(energy,energy_sum,(ny-1)*ny/2,MPI_DOUBLE,MPI_SUM,0,MPI_COMM_WORLD,ierr)
  ! call mpi_reduce(helicity,helicity_sum,(ny-1)*ny/2,MPI_DOUBLE,MPI_SUM,0,MPI_COMM_WORLD,ierr)

  ! if(my_id.eq.0)then
  !   open(44,file='invarians.dat')
  !   write(44,*) 'variables= sacle,position,energy,helicity'
  !   write(44,*) 'zone j=',ny-1, 'k=',ny/2
  !   do k=1,ny/2 !position
  !     do j=1,ny-1 !scale
  !       scale=sqrt((Ayy(1,j+1,1)-Ayy(1,1,1))**2 + (Azz(1,j+1,1)-Azz(1,1,1))**2)
  !       position=sqrt(Ayy(1,2*k,1)**2 + Azz(1,2*k,1)**2)
  !       write(44,'(10f32.8)')scale,position,energy(j,k),helicity(j,k)
  !     end do
  !   end do
  !   close(44)
  ! end if
  ...
  do while( num .ne. 0)
    ...
    read(*,*) num
    if(num .eq. 1) then
      call write_i(nx,ny,nz,Axx(1:nx,1:ny,1:nz),Ayy(1:nx,1:ny,1:nz),Azz(1:nx,1:ny,1:nz),d,u,v,w,T,P00)
    else if (num .eq. 2) then
      call write_j(nx,ny,nz,Axx(1:nx,1:ny,1:nz),Ayy(1:nx,1:ny,1:nz),Azz(1:nx,1:ny,1:nz),d,u,v,w,T,P00)
    else if (num .eq. 3) then
      call write_k(nx,ny,nz,Axx(1:nx,1:ny,1:nz),Ayy(1:nx,1:ny,1:nz),Azz(1:nx,1:ny,1:nz),d,u,v,w,T,P00)
    else if (num .eq. 4) then
      call write_3D(nx,ny,nz,Axx(1:nx,1:ny,1:nz),Ayy(1:nx,1:ny,1:nz),Azz(1:nx,1:ny,1:nz),d,u,v,w,T,P00,Ma,Q_criterion)
    endif
  enddo
  deallocate(d_av_x, u_av_x, v_av_x, w_av_x, T_av_x)
  deallocate(d_av_xz,u_av_xz,v_av_xz, w_av_xz,T_av_xz)
  deallocate(temp,u_wavelet,v_wavelet,w_wavelet,vor_x_wavelet,vor_y_wavelet,vor_z_wavelet,energy,helicity,energy_sum,helicity_sum)
...
!==================================
subroutine read3d(no,nx,ny,nz,u,id)
  implicit none
  integer:: no,nx,ny,nz,k,id
  real*8:: u(nx,ny,nz)
  if(id.eq.0)print*, "read 3d data ..."
  do k=1,nz
    read (no)  u(:,:,k)
  enddo
end
...
!--------------------------------------------
subroutine read_mesh(nx,ny,nz,x,y,z,id)
  implicit none
  integer:: nx,ny,nz,mesh_type,i,j,k,id
  ...
  if(mesh_type ==GRID1D) then
    ...
    call read3d(56,nx,ny,nz,x,id)
    call read3d(56,nx,ny,nz,y,id)
    call read3d(56,nx,ny,nz,z,id)
  endif
  close(56)
...
! spectral analysis
subroutine fourier_transform_r(nx,ny,nz,x,y,z,fin,fout)
...
subroutine wavelet_transform(ny,x,y,z,fin,fout)
  integer:: nx,ny,nz,i,j,kmax
  real*8,dimension(ny)::fin,x,y,z,r,dr
  real*8,dimension(ny-1,ny/2)::fout
  real*8::rmin,rmax,dr_max,pi=3.14159265358979323846,position,scale,mexican_hat_wavelet,psi
  ...
  do i = 1 , ny-1   ! scale
    ...
    do j = 1, ny/2    ! position
      position = r(j*2)
      do k = 1, ny-1
        psi = mexican_hat_wavelet(((r(k)+r(k+1))/2-position)/scale)
        fout(i,j) = fout(i,j) + scale**(-0.5)*psi*fin(k)*(r(k+1)-r(k))
      end do
    end do
  end do
end subroutine
...
!--------- i+ boundary ------------------------
  if(npx .eq. npx0-1 .and. Para%Iperiodic_X .eq. 0) then
    do k=1,nz
      do j=1,ny
        fx(nx  ,j,k)=a1*(f(3,j,k)-f(nx-3,j,k)) +a2*(f(2,j,k)-f(nx-2,j,k)) +a3*(f(1,j,k)-f(nx-1,j,k))
        fx(nx-1,j,k)=a1*(f(2,j,k)-f(nx-4,j,k)) +a2*(f(1,j,k)-f(nx-3,j,k)) +a3*(f(nx,j,k)-f(nx-2,j,k))
        fx(nx-2,j,k)=a1*(f(1,j,k)-f(nx-5,j,k)) +a2*(f(nx,j,k)-f(nx-4,j,k)) +a3*(f(nx-1,j,k)-f(nx-3,j,k))
      enddo
    enddo
    if(Num_Scheme == OCFD_Scheme_CD8) then
      do k=1,nz
        do j=1,ny
          fx(nx,j,k)=c1*(f(1,j,k)-f(nx-1,j,k)) +c2*(f(2,j,k)-f(nx-2,j,k))  &
            +c3*(f(3,j,k)-f(nx-3,j,k)) +c4*(f(4,j,k)-f(nx-4,j,k))
          fx(nx-1,j,k)=c1*(f(nx,j,k)-f(nx-2,j,k)) +c2*(f(1,j,k)-f(nx-3,j,k))  &
            +c3*(f(2,j,k)-f(nx-4,j,k)) +c4*(f(3,j,k)-f(nx-5,j,k))
          fx(nx-2,j,k)=c1*(f(nx-1,j,k)-f(nx-3,j,k)) +c2*(f(nx,j,k)-f(nx-4,j,k))  &
            +c3*(f(1,j,k)-f(nx-5,j,k)) +c4*(f(2,j,k)-f(nx-6,j,k))
          fx(nx-3,j,k)=c1*(f(nx-2,j,k)-f(nx-4,j,k)) +c2*(f(nx-1,j,k)-f(nx-5,j,k))  &
            +c3*(f(nx,j,k)-f(nx-6,j,k)) +c4*(f(1,j,k)-f(nx-7,j,k))
        enddo
      enddo
    endif
  endif
...
!---------- k- boundary  ---------------------------
  if(npz .eq. 0 .and. Para%Iperiodic_Z .eq. 0) then
    do j=1,ny
      do i=1,nx
        fz(i,j,1)=a1*(f(i,j,4)-f(i,j,nz-2)) +a2*(f(i,j,3)-f(i,j,nz-1)) +a3*(f(i,j,2)-f(i,j,nz))
        fz(i,j,2)=a1*(f(i,j,5)-f(i,j,nz-1)) +a2*(f(i,j,4)-f(i,j,nz  )) +a3*(f(i,j,3)-f(i,j,1))
        fz(i,j,3)=a1*(f(i,j,6)-f(i,j,nz  )) +a2*(f(i,j,5)-f(i,j,1   )) +a3*(f(i,j,4)-f(i,j,2))
      enddo
    enddo
    if(Num_Scheme == OCFD_Scheme_CD8) then
      do j=1,ny
        do i=1,nx
          fz(i,j,1)=c1*(f(i,j,2)-f(i,j,nz)) +c2*(f(i,j,3)-f(i,j,nz-1))  &
            +c3*(f(i,j,4)-f(i,j,nz-2)) +c4*(f(i,j,5)-f(i,j,nz-3))
          fz(i,j,2)=c1*(f(i,j,3)-f(i,j,1)) +c2*(f(i,j,4)-f(i,j,nz))  &
            +c3*(f(i,j,5)-f(i,j,nz-1)) +c4*(f(i,j,6)-f(i,j,nz-2))
          fz(i,j,3)=c1*(f(i,j,4)-f(i,j,2)) +c2*(f(i,j,5)-f(i,j,1))  &
            +c3*(f(i,j,6)-f(i,j,nz)) +c4*(f(i,j,7)-f(i,j,nz-1))
          fz(i,j,4)=c1*(f(i,j,5)-f(i,j,3)) +c2*(f(i,j,6)-f(i,j,2))  &
            +c3*(f(i,j,7)-f(i,j,1)) +c4*(f(i,j,8)-f(i,j,nz))
        enddo
      enddo
    endif
  endif
!--------- k+ boundary ------------------------
  if(npz .eq. npz0-1 .and. Para%Iperiodic_Z .eq. 0) then
    do j=1,ny
      do i=1,nx
        fz(i,j,nz  )=a1*(f(i,j,3)-f(i,j,nz-3)) +a2*(f(i,j,2 )-f(i,j,nz-2)) +a3*(f(i,j,1   )-f(i,j,nz-1))
        fz(i,j,nz-1)=a1*(f(i,j,2)-f(i,j,nz-4)) +a2*(f(i,j,1 )-f(i,j,nz-3)) +a3*(f(i,j,nz  )-f(i,j,nz-2))
        fz(i,j,nz-2)=a1*(f(i,j,1)-f(i,j,nz-5)) +a2*(f(i,j,nz)-f(i,j,nz-4)) +a3*(f(i,j,nz-1)-f(i,j,nz-3))
      enddo
    enddo
    if(Num_Scheme == OCFD_Scheme_CD8) then
      do j=1,ny
        do i=1,nx
          fz(i,j,nz  )=c1*(f(i,j,1)-f(i,j,nz-1)) +c2*(f(i,j,2)-f(i,j,nz-2))  &
            +c3*(f(i,j,3)-f(i,j,nz-3)) +c4*(f(i,j,4)-f(i,j,nz-4))
          fz(i,j,nz-1)=c1*(f(i,j,nz)-f(i,j,nz-2)) +c2*(f(i,j,1)-f(i,j,nz-3))  &
            +c3*(f(i,j,2)-f(i,j,nz-4)) +c4*(f(i,j,3)-f(i,j,nz-5))
          fz(i,j,nz-2)=c1*(f(i,j,nz-1)-f(i,j,nz-3)) +c2*(f(i,j,nz)-f(i,j,nz-4))  &
            +c3*(f(i,j,1)-f(i,j,nz-5)) +c4*(f(i,j,2)-f(i,j,nz-6))
          fz(i,j,nz-3)=c1*(f(i,j,nz-2)-f(i,j,nz-4)) +c2*(f(i,j,nz-1)-f(i,j,nz-5))  &
            +c3*(f(i,j,nz)-f(i,j,nz-6)) +c4*(f(i,j,1)-f(i,j,nz-7))
        enddo
      enddo
    endif
  endif
